...
class SearchAdapter(private val originalItems: MutableList<BlogItemModel>) :
    RecyclerView.Adapter<SearchAdapter.BlogViewHolder>() {

    private val databaseReference: DatabaseReference = FirebaseDatabase.getInstance().reference
    private val currentUser = FirebaseAuth.getInstance().currentUser

    private val items: MutableList<BlogItemModel> = mutableListOf()

    init {
        // Initialize items with the original data
    ...
    fun setItems(newItems: List<BlogItemModel>) {
        items.clear()
    ...
    fun filter(query: String) {
        ...
        if (query.trim().isEmpty()) {
            ...
            // If there is a query, filter items based on the search logic
            val filteredItems = originalItems.filter { blog ->
                blog.heading?.toLowerCase()?.contains(query.toLowerCase()) == true ||
                        blog.blogText?.toLowerCase()?.contains(query.toLowerCase()) == true
            }
